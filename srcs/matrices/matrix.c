...
/*   Updated: 2021/01/20 16:24:29 by lessie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec   *vec_transform(t_vec *vec, float **matrix)
{
    ...
    while (i < 4)
    {
        homogen_transformed[0][i] = vec_homogen[0][0] * matrix[i][0] +
                                    vec_homogen[0][1] * matrix[i][1] +
                                    vec_homogen[0][2] * matrix[i][2] +
                                    vec_homogen[0][3] * matrix[i][3];
        i++;
    }
    vec_transformed = new_vec(homogen_transformed[0][0], homogen_transformed[0][1], homogen_transformed[0][2]);
