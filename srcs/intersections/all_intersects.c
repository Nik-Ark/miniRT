...
/*   Updated: 2021/02/17 12:53:02 by lessie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		trace_ray(t_closest *closest, t_scene *scene, t_vec *origin, t_vec *dir)
{
	...
	if (closest->type != 0)
		rgb = get_lightning(closest, scene, dir); // check color in order with type
	return (rgb);
}
...
void	check_all(t_closest *closest, t_scene *scene, t_vec *origin, t_vec *dir)
{
	...
	//check every other figure types...
}
...
int		get_lightning(t_closest *closest, t_scene *scene, t_vec *dir)
{
	...
	rgb = 0;
	rgb = closest->type == SPHERE ? get_sphere_light(closest, scene, dir, scene->colb) : rgb;
	rgb = closest->type == PLANE ? get_plane_light(closest, scene, dir, scene->colb) : rgb;
	rgb = closest->type == TRIANGLE ? get_triang_light(closest, scene, dir, scene->colb) : rgb;
    rgb = closest->type == CYLINDER ? get_cylinder_light(closest, scene, dir, scene->colb) : rgb;
	return (rgb);
}
...
int		add_figure_color(t_colb *colb, int figure_rgb, float to_cam_amb)
{
	...
	r = get_r(figure_rgb) * (colb->rgb->u + (colb->amb->u / (to_cam_amb * to_cam_amb)));
	r = r > 255 ? 255 : r;
	r = r < 0 ? 0 : r;
	g = get_g(figure_rgb) * (colb->rgb->v + (colb->amb->v / (to_cam_amb * to_cam_amb)));
	g = g > 255 ? 255 : g;
	g = g < 0 ? 0 : g;
	b = get_b(figure_rgb) * (colb->rgb->n + (colb->amb->n / (to_cam_amb * to_cam_amb)));
	b = b > 255 ? 255 : b;
	b = b < 0 ? 0 : b;
