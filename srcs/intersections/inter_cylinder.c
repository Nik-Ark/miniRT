...
float   trim_cyl(t_vec *orig, t_vec *dir, t_cyl *cyl, const float *dist)
{
    float   closest;
...
int     in_height(t_vec *orig, t_vec *dir, t_cyl *cyl, float closest)
{
    ...
    normal = vec_subtract(point, p_on_height);
    vec_normalize(normal);
    cyl->norm->u = normal->n < 0 ? normal->u * (-1) : normal->u;
    cyl->norm->v = normal->n < 0 ? normal->v * (-1) : normal->v;
    cyl->norm->n = normal->n < 0 ? normal->n * (-1) : normal->n;
    free(hit_p);
    free(point);
    free(hit_proec);
    free(p_on_height);
