...
/*   Updated: 2021/04/20 12:38:54 by lessie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     check_append_square(t_scene **scene, float *vector, float length, int rgb)
{
    t_vec		*center;
    t_vec		*normal;
    t_triang	*triang;
    ...
    t_vec		**sq_vert;
/*
	left_down(vert_1)(vert_4); 	// sq_vert[0] + center
	left_up(vert_2);          	// sq_vert[1] + center
	right_up(vert_3)(vert_5);	// sq_vert[2] + center
	right_down(vert_6);     	// sq_vert[3] + center
*/

    center = new_vec(vector[0], vector[1], vector[2]);
    normal = new_vec(vector[3], vector[4], vector[5]);
    sq_vert = get_sq_vert(normal, length); 				// SHOULD BE PARSED LENGTH  BUT NOT NECESSARY
    triang = new_triang(vec_addition(sq_vert[0], center),
                        vec_addition(sq_vert[1], center), vec_addition(sq_vert[2], center), rgb);
    triang->next = new_triang(vec_addition(sq_vert[0], center),
                              vec_addition(sq_vert[2], center), vec_addition(sq_vert[3], center), rgb);
    tmp = (*scene)->tr_lst;
    if (tmp)
        while (tmp->next)
            tmp = tmp->next;
...
t_vec	**get_sq_vert(t_vec *normal, float length)
{
    ...
    printf("Square normal after normalization: u = %f v = %f n = %f\n", normal->u, normal->v, normal->n);
